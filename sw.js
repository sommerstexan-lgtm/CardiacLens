const CACHE_NAME = 'cardiaclens-v9.10.277-guarddog-cleanup-safe';
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.indexOf('cardiaclens-') === 0 && key !== CACHE_NAME).map(key => caches.delete(key))))
...
self.addEventListener('fetch', event => {
  ...
  if (isDocument || url.pathname.endsWith('/version.json') || url.pathname === '/version.json') {
    event.respondWith(fetch(req, { cache: 'reload' }).then(response => {
